...
class StocksFinancialsController extends Controller {
    ...
    public function updateFinancialDataQuarter($stock) {
        $ieTrading = new IexTrading();
        ...
        foreach ($response as $financial) {
                ...
                if (isset($financial->reportDate)) {
                    $newStockFinancialsQuarter->report_date = $financial->reportDate;
                }

                if (isset($financial->grossProfit)) {
                    $newStockFinancialsQuarter->gross_profit = $financial->grossProfit;
                }

                if (isset($financial->costOfRevenue)) {
                    $newStockFinancialsQuarter->cost_of_revenue = $financial->costOfRevenue;
                }

                if (isset($financial->operatingRevenue)) {
                    $newStockFinancialsQuarter->operating_revenue = $financial->operatingRevenue;
                }

                if (isset($financial->totalRevenue)) {
                    $newStockFinancialsQuarter->total_revenue = $financial->totalRevenue;
                }

                if (isset($financial->operatingIncome)) {
                    $newStockFinancialsQuarter->operating_income = $financial->operatingIncome;
                }

                if (isset($financial->netIncome)) {
                    $newStockFinancialsQuarter->net_income = $financial->netIncome;
                }

                if (isset($financial->researchAndDevelopment)) {
                    $newStockFinancialsQuarter->research_and_development = $financial->researchAndDevelopment;
                }

                if (isset($financial->operatingExpense)) {
                    $newStockFinancialsQuarter->operating_expense = $financial->operatingExpense;
                }

                if (isset($financial->currentAssets)) {
                    $newStockFinancialsQuarter->current_assets = $financial->currentAssets;
                }

                if (isset($financial->totalAssets)) {
                    $newStockFinancialsQuarter->total_assets = $financial->totalAssets;
                }

                if (isset($financial->totalLiabilities)) {
                    $newStockFinancialsQuarter->total_liabilities = $financial->totalLiabilities;
                }

                if (isset($financial->currentCash)) {
                    $newStockFinancialsQuarter->current_cash = $financial->currentCash;
                }

                if (isset($financial->currentDebt)) {
                    $newStockFinancialsQuarter->current_debt = $financial->currentDebt;
                }

                if (isset($financial->totalCash)) {
                    $newStockFinancialsQuarter->total_cash = $financial->totalCash;
                }

                if (isset($financial->totalDebt)) {
                    $newStockFinancialsQuarter->total_debt = $financial->totalDebt;
                }

                if (isset($financial->shareholderEquity)) {
                    $newStockFinancialsQuarter->shareholder_equity = $financial->shareholderEquity;
                }

                if (isset($financial->cashChange)) {
                    $newStockFinancialsQuarter->cash_change = $financial->cashChange;
                }

                if (isset($financial->cashFlow)) {
                    $newStockFinancialsQuarter->cash_flow = $financial->cashFlow;
                }

                if (isset($financial->operatingGainsLosses)) {
                    $newStockFinancialsQuarter->operating_gains_losses = $financial->operatingGainsLosses;
                }
                $newStockFinancialsQuarter->save();
