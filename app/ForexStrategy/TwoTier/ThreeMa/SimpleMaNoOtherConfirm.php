...
namespace App\ForexStrategy\TwoTier\ThreeMa;
use \Log;
use \App\IndicatorEvents\Momentum;

class SimpleMaNoOtherConfirm extends \App\ForexStrategy\Strategy  {
    ...
    public function checkForNewPosition() {
        ...
        $this->decisionIndicators['slowThreeLine'] = $momentumEvents->threeMASimple($this->fastMaLength, $this->mediumMaLength, $this->slowMaLength, $this->slowRates);

        $this->decisionIndicators['fastThreeLine'] = $momentumEvents->threeMASimple($this->fastMaLength, $this->mediumMaLength, $this->slowMaLength, $this->rates);

        $this->ratesPips = $this->getRatesInPips($this->slowRates);

        $this->decisionIndicators['slowHmaMeetsCutoffSlope'] = $momentumEvents->hmaMeetsCutoffPipSlope(9, 10, $this->ratesPips);
