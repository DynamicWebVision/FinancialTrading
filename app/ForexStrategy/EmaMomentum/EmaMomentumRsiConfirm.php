...
class EmaMomentumRsiConfirm extends \App\ForexStrategy\Strategy  {
    ...
    public function decision() {
        ...
        if ($this->decisionIndicators['emaCrossover'] == "crossedAbove" && $this->decisionIndicators['rsi'] <= (100 - $this->rsiCutoff)) {
            Log::warning($this->runId.': NEW LONG POSITION');
            return "long";
        }
        elseif ($this->decisionIndicators['emaCrossover'] == "crossedBelow"  && $this->decisionIndicators['rsi'] >= $this->rsiCutoff) {
    ...
    public function checkForNewPosition($rates) {
        ...
        $this->decisionIndicators['rsi'] = $this->indicators->rsi($rates, $this->rsiLength);
