...
namespace App\ForexStrategy;
use \Log;

class EmaMomentumWithConfirmation extends \App\ForexStrategy\Strategy  {
    ...
    public $confirmationEma;
    public $confirmationEmaLinRegLength;
    public $confirmLinRegCutoff;
    ...
    public function decision() {
        ...
        if ($this->decisionIndicators['emaCrossover'] == "crossedAbove" && $this->decisionIndicators['confirmLinReg'] > $this->confirmLinRegCutoff) {
            Log::warning($this->runId.': NEW LONG POSITION');
            return "long";
        }
        elseif ($this->decisionIndicators['emaCrossover'] == "crossedBelow" && $this->decisionIndicators['confirmLinReg'] > $this->confirmLinRegCutoff*-1) {
    ...
    public function checkForNewPosition($rates) {
        ...
        $this->decisionIndicators['confirmationEma'] = $this->indicators->ema($rates, $this->confirmationEma);

        $pipConfirmationEmaArray = array_map(function($v) {
            return $v/$this->exchange->pip;
        }, $this->decisionIndicators['confirmationEma']);

        $this->decisionIndicators['confirmLinReg'] = $this->indicators->linearRegression($pipConfirmationEmaArray, $this->confirmationEmaLinRegLength);
