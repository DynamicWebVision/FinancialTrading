<?php namespace App\ForexBackTest\BackTestToBeProcessed\ForexStrategy\HmaPricePoint;

/**********************
HmaPricePoint Backtest Variable Definitions
...
use \App\ForexStrategy\HmaPricePoint\HmaPPSimple;
use \App\ForexStrategy\HmaPricePoint\HmaPpTrBeSl;
//END STRATEGY DECLARATIONS

class HmaPricePointBackTestToBeProcessed extends \App\ForexBackTest\BackTestToBeProcessed\Base
{
    ...
    public function callProcess() {
        ...
        ******************************/
        ...
        elseif ($this->server->strategy_iteration == 'HMA_PP_SIMPLE') {
            $backTest->rateLevel = 'both';
        
            $strategy = new HmaPPSimple(1,1,true);
        ...
        elseif ($this->server->strategy_iteration == 'HMA_PP_TR_BE_SL') {
            $backTest->rateLevel = 'both';
        
            $strategy = new HmaPpTrBeSl(1,1,true);

            $strategy->orderType = 'MARKET_IF_TOUCHED';
        
            //$strategy->orderType = 'MARKET_IF_TOUCHED';
            $strategy->hmaLength = intval($this->backTestToBeProcessed->variable_1);
            $strategy->trueRangeLength = intval($this->backTestToBeProcessed->variable_2);
            $strategy->trueRangeMultiplierOpen = intval($this->backTestToBeProcessed->variable_3);
